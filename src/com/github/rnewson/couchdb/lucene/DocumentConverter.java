...
public final class DocumentConverter {
	...
	private static final Collection<Document> NO_DOCUMENTS = Collections
			.emptyList();
	private static final String LOG_TAG = DocumentConverter.class.getName();

	private final Context context;
	private final Function viewFun;
	private final ScriptableObject scope;

	public DocumentConverter(final Context context, final View view)
		...
		try {
			...
			Log.e(LOG_TAG, "View code for " + view + " does not compile.");
			throw e;
		}
	}

	public Collection<Document> convert(final CouchDocument doc,
			final ViewSettings defaults, final Database database)
		...
		if (result instanceof RhinoDocument) {
			...
			final Document document = rhinoDocument.toDocument(doc.getId(),
					defaults, database);
