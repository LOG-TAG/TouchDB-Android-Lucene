...
public enum Analyzers {
	...
	PERFIELD {
		...
		public Analyzer newAnalyzer(final String args) throws JSONException {
			...
			final Analyzer defaultAnalyzer = Analyzers.getAnalyzer(json
					.optString(Constants.DEFAULT_FIELD, "standard"));
			final PerFieldAnalyzerWrapper result = new PerFieldAnalyzerWrapper(
					defaultAnalyzer);
			...
			while (it.hasNext()) {
				...
				if (Constants.DEFAULT_FIELD.equals(key))
					continue;
				result.addAnalyzer(key,
						Analyzers.getAnalyzer(json.getString(key)));
			}
			return result;
		}
	},
